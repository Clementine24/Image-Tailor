...
if __name__ == "__main__":
    ...
    axes[0].set_title("the pic with linear blending")
    axes[0].set_axis_off()
    axes[0].imshow(stitched_image_linear[:, :, ::-1].astype(int))
    axes[1].set_title("the pic with no blending")
    axes[1].set_axis_off()
    axes[1].imshow(stitched_image_no[:, :, ::-1].astype(int))
    fig.show()
